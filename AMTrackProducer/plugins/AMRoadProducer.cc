#ifndef NTupleTools_AMRoadProducer_h_
...
#include<TH2F.h>
#include<map>
class AMRoadProducer : public edm::EDProducer {
  public:
    explicit AMRoadProducer(const edm::ParameterSet&);
...
void AMRoadProducer::endJob(){
delete ttmap_;
delete fin;
}
AMRoadProducer::AMRoadProducer(const edm::ParameterSet& iConfig) {
     StubsTag_ =(iConfig.getParameter<edm::InputTag>("inputTagStub"));
     produces< std::vector< TTTrack< Ref_PixelDigi_ > > >( "Level1Roads" ).setBranchAlias("Level1Roads");

}

void AMRoadProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    //std::auto_ptr<std::vector<float> > dummies(new std::vector<float>());
//  typedef TTStubAssociationMap<Ref_PixelDigi_> assocmap_stub;

  std::auto_ptr< std::vector< TTTrack< Ref_PixelDigi_ > > > L1TkRoadsForOutput( new std::vector< TTTrack< Ref_PixelDigi_ > > );
...
//		std::cout<<" Other Towers "<<OtherTower[i]<<std::endl;
   ...
   // std::cout<<"Roads "<<roads.size()<<std::endl;i
   for (unsigned iroad=0; iroad<roads.size(); ++iroad) { 
        TTTrack<Ref_PixelDigi_> aTrack;
	std::vector<std::vector<unsigned> >StubsRefIndex=roads[iroad].stubRefs;
	for (unsigned ilayer=0; ilayer<StubsRefIndex.size(); ++ilayer) {
        unsigned sindex = 0;
        ...
        for ( inputIter = pixelDigiTTStubs->begin(); inputIter != pixelDigiTTStubs->end(); ++inputIter ){
                for ( stubIter = inputIter->begin(); stubIter != inputIter->end(); ++stubIter ){
                        std::vector<unsigned>::iterator it=std::find(StubsRefIndex[ilayer].begin(), StubsRefIndex[ilayer].end(),sindex);
                        if(it!=StubsRefIndex[ilayer].end()){
...
*/
iEvent.put( L1TkRoadsForOutput, "Level1Roads");
...
double AMRoadProducer::genTrackDistanceTransverse(const double &pt, const double &phi0, const double &d0,
                                                 const int charge, const double &B, const double &phi, const double &R) 
{
...
double AMRoadProducer::genTrackDistanceLongitudinal(const double &z0, const double &cotTheta, const double &pt, const double &d0,
const int charge, const double &B, const double &R, const double &z) {
...
int  AMRoadProducer::PatternBankMap(float eta, float phi){
int pb=0;
...
void AMRoadProducer::initOptions(ProgramOption &option, int PBIndex){
...
DEFINE_FWK_MODULE(AMRoadProducer);
